...
Runs the fixture app through thirteen scenarios — unchanged (twice, for
...
links to beside four declared addresses that must stay silent, and three selects
that are identical in the DOM and must not be reported identically. 76 checks,
all passing as of the last commit.

The last four scenarios are built the same way on purpose. Each of those fixes
...
| App Atlas web | React + React Flow SPA | 40 (budget) | 197 | 0 |

Runs that stop at a budget say so and report what they never reached, rather
than implying they covered everything.

That App Atlas row read 83 walked and 0 findings for a long time, and both
numbers had drifted from the truth in opposite directions: the selector and
`inert` fixes had more than doubled what the walk reaches, and the walk that
reached further was reporting four dead controls that all worked. Three were the
canvas zoom controls and one was the breadcrumb for the current page — the two
classes below. It now walks 197 interactions and reports nothing, over 103
navigations, 85 state changes, 3 `visual-only` and 6 benign no-effects.

Worth saying plainly, since a table of zeroes invites the wrong reading: a clean
run here is evidence the false-positive classes are fixed, not evidence the app
is perfect. This app walks clean and always has. It is a good test of whether
clickgraph cries wolf and a poor one of whether it catches wolves — which is
what the fixture, with its planted defects, is for.
...
- **An app that says "you are here" in CSS is still saying it.** The last finding standing on App Atlas was a button named `@app-atlas/cli`, on the screen whose own URL is `app:@app-atlas/cli`. Driving it by hand changes nothing at all — not the URL, not the text, not one byte of `outerHTML` — because it is the breadcrumb for the page you are already on, and its class is `crumb is-current`. Correct behavior, reported as a dead control. It escaped all three existing rules at once: no `aria-current` to read, not a link so there is no href to compare against the current URL, and the walk had arrived by a different control so it was never "already applied". The fix reads the class, but a class name is the app's private vocabulary — `active` sitting on a genuinely broken button would excuse the bug, which is the false negative this project spends findings to avoid. So the class never counts on its own: the control must also name the location the browser is already at. Two independent signals agreeing is the whole safeguard.
...
- **A select whose only effect is its own value is not a dead control — and the value cannot be the thing that proves it.** A controlled React `<select>` reported NO EFFECT on every choice: React writes the value as a *property*, so no attribute mutates, no text is rewritten, and the closed control's new text is painted by the browser rather than found in the DOM. Nothing a snapshot compares changes. The obvious fix — treat "the value changed" as working — is wrong, and the fixture is built to prove it: the browser sets the value whether or not the app is listening, so that rule is equally true of a filter select wired to nothing, and it would excuse the exact defect this tool exists to find. The signal that separates them is whether anything can consume the value. A select inside a form with a submit has a consumer by construction, so its silence at selection time is not evidence of anything and the submit is where the proof lives; a select with nothing to submit it has no later moment, so an immediate effect is the only effect it could have and silence is the whole defect. That is `value-set`, and it is deliberately weaker than `state-changed` — what is known is that the control accepted the value, not that anything read it. The guard on the rule is the third case: a controlled select whose handler never commits snaps back to what state still says, which is React restoring the value it was not told to change, and the result is a select that cannot be changed at all. It sits in a form like the working one, so membership had to stay necessary without becoming sufficient — the value is read back, and a control that did not keep what it was given is still reported. Reading it back also fixed a report that lied: the finding used to say the option had been `set to` a value the control had refused, and now says `asked for`.
- **Filling a form and proving a select responds are different questions.** Choosing "an option it is not already showing" is right for testing a select and wrong for filling a form, and a walk does not reload between actions in one state — so a select still shows whatever the previous action chose, and "something else" comes back around to the `<option value="">Pick one…</option>` placeholder. That is the single value that makes a required form invalid, so the submit was skipped as `needs-input` and the button behind it was never tested. Every form with a required select was affected. The symptom is silence rather than a wrong finding, which is why it survived: the run says it declined to test something, and a reader skims past a skip.
