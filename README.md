...
- **The DOM lies about a form's own id.** `HTMLFormElement` exposes its controls as named properties and those *override built-ins*, so a form containing `<input name="id">` answers `form.id` with the input element rather than with its id string. Deriving a fallback CSS path walks ancestors reading that property, met an element instead of a string, and `id.startsWith is not a function` came out of a page evaluate and ended the whole run — no graph, no baseline (issue #55). It surfaced on the first walk of the first third-party app the tool was pointed at, because `name="id"` is every admin filter and every search-by-id box. The id is read off the attribute now, and the stable-id check refuses a non-string rather than trusting one: the cost of being wrong inside a page evaluate is not a bad selector, it is a lost walk. The quieter half of the same bug never threw at all — a scroll-pane key concatenated the element and silently keyed on `[object HTMLInputElement]`.
